# Update angular.json
$angularJsonContent = Get-Content -Path "angular.json" -Raw
$angularJson = $angularJsonContent | ConvertFrom-Json
$angularJson | ConvertTo-Json -Depth 100 | Set-Content -Path "angular.json"
...
# Create polyfills.ts if it doesn't exist
if (-not (Test-Path "src\polyfills.ts")) {
    $polyfillsContent = @"
/**
...
Write-Host "Project structure has been fixed and configuration files have been updated."